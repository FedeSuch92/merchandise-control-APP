...
/*************** PR1 EX2  *****************/
    ...
    printf("Material id (positive integer less or equal than %d)\n>> ", MAX_MATERIALS);
    scanf("%d", &number);
    ...
    printf("Material name (max 20 char, no spaces):\n>> ");
    scanf("%s",buffer);
    ...
    printf("Material type (a number being 0=CONCRETE, 1=GLASS, 2=WOOD, 3=BRICK, 4=TUBES, 5=ALUMINUM, 6=SAND, 7=FOAM, 8=PLADUR, 9=TILES, 10=INSULATION, 11=GAS, 12=WATER, 13=OTHER_MATERIALS):\n>> ");
    scanf("%d",&number);
    ...
    printf("Material weight: \n>> ");
    scanf("%f", &(material->weight));
    
    printf("Material height: \n>> ");
    scanf("%f", &(material->height));   
    
    printf("Material volume: \n>> ");
    scanf("%f", &(material->volume));
    
    printf("Layers (Integer greater or equal than 0):\n>> ");
    scanf("%d", &number);
    if (number < 0){
        *retVal = ERROR;
    }else{
        material->layers = number;
    }
